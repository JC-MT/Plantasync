...
export function meta({ data }: { data: { plant: Plant; actions: Action[] } }) {
  return [
...
export async function loader({
  ...
  const actionsPromise = getData(
    `actions?plant_id=eq.${params.id}${searchParams.get("filter") ? `&type=eq.${searchParams.get("filter")}` : ""}`
  )
...
export default function Detail({
  ...
  loaderData: { plant: Plant; actions: Promise<Action[]> };
}) {
